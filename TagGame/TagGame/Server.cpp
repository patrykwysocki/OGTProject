...
bool Server::ProcessPacket(int ID, Packet _packettype)
{
	...
	{
		std::string Vector;
		if (!GetPlayerCollision(ID, Vector))
			return false;
		...
		for (int i = 0; i < TotalConnections; i++)
		{
			if (i == ID) //If connection is the user who sent the message...
				continue;//Skip to the next user since there is no purpose in sending the message back to the user who sent it.
			if (!SendPlayerCollision(i, Vector)) //Send message to connection at index i, if message fails to be sent...
			{
				std::cout << "Failed to send Data from client ID: " << ID << " to client ID: " << i << std::endl;
			}
		}
